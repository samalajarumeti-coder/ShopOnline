// Create tables in Supabase using service role key
...
async function checkTables() {
  ...
  const { data: catCheck, error } = await supabase.from('categories').select('id').limit(1)
  
  if (error && error.code === '42P01') {
    console.log('❌ Tables do not exist.')
    console.log('\n📋 Please run the SQL migration manually in Supabase Dashboard:')
    console.log('📍 Go to: https://supabase.com/dashboard/project/hdcigjthmiohhnfcqnvl/sql/new')
